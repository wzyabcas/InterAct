...
smplh_model_female = smplx.create(MODEL_PATH, model_type='smplh',
                        ...
                        use_pca=False,
                        ext='pkl',flat_hand_mean=True).to(device)
sbj_m_all={'male':smplh_model_male,'female':smplh_model_female}
...
rhand_idx=np.load('./assets/smplh_hand_index/rhand_smplh_ids.npy')
...
for i in range(5):
    for j in range(3):
        RHAND_SMALL_INDEXES_DETAILED.append(np.load(f'./assets/smplh_hand_index/hand_778_small_{i}_{j}.npy'))
# HALF PALM

RHAND_SMALL_INDEXES_DETAILED.append(np.load(f'./assets/smplh_hand_index/hand_778_small_5.npy'))
...
for i in range(5):
    for j in range(3):
        LHAND_SMALL_INDEXES_DETAILED.append(np.load(f'./assets/smplh_hand_index/lhand_778_small_{i}_{j}.npy'))
LHAND_SMALL_INDEXES_DETAILED.append(np.load(f'./assets/smplh_hand_index/lhand_778_small_5.npy'))
...
def optimize1(name,fn,dataset_name):
    ...
    OBJ_PATH='./data/behave/objects'
    obj_simplified_path=os.path.join(OBJ_PATH,SIMPLIFIED_MESH[obj_name])
    obj_sim_path=os.path.join(OBJ_PATH,obj_name,obj_name+'.obj')
    obj_dir_name=os.path.join(OBJ_PATH,obj_name)
    #MMESH=trimesh.load(os.path.join(obj_dir_name,obj_name+'.obj'))
    MMESH=trimesh.load(obj_sim_path)
    MMESH_simple=trimesh.load(obj_simplified_path)
    #MMESH=trimesh.load(os.path.join(obj_dir_name,obj_name+'.obj'))
    verts_obj=np.array(MMESH.vertices)
    faces_obj=np.array(MMESH.faces)
    MEAN=np.mean(verts_obj,0)
    verts_obj=verts_obj-MEAN
    verts_obj_simple=np.array(MMESH_simple.vertices)
    verts_obj_simple=verts_obj_simple-MEAN
    ...
    smplx_output = sbj_m(body_pose=body_pose,
                                ...
                                left_hand_pose=hand_pose_tensor[:,:45],
                                right_hand_pose=hand_pose_tensor[:,45:],
                                betas=betas_tensor,
                                transl=trans_tensor)
    verts_sbj_mean = (smplx_output.vertices)
    ...
    verts_obj_simple=obj_forward(torch.from_numpy(verts_obj_simple).float().to(device),obj_6d_tensor,obj_trans_tensor)
    rhand_idx=np.load('./assets/smplh_hand_index/rhand_smplh_ids.npy')
    lhand_idx_path='./assets/smplh_hand_index/lhand_smplh_ids.npy'
    lhand_idx=np.load(lhand_idx_path)
    all_human_faces=sbj_m.faces

    human_normals_mean=vertex_normals(verts_sbj_mean,torch.tensor(all_human_faces.astype(np.float32)).unsqueeze(0).repeat(verts_sbj_mean.shape[0],1,1).to(device))
    h2o_signed_mean=point2point_signed(verts_sbj_mean[:,rhand_idx],verts_obj,x_normals=human_normals_mean[:,rhand_idx])[0]
    
    
    ## Weight for pre contact -> contact -> post contact
    min_distance_mean_hand=torch.min(h2o_signed_mean,dim=1)[0]
    
    ## threshold for contact and post contact
    THR=0.03
    MASK1=min_distance_mean_hand<=THR
    thresh2=0.10
    k_l=1/(THR-thresh2)
    b_l=thresh2/(thresh2-THR)
    ...
    ## Linear weight for pre contacting - > contact
    ...
    h2o_signed_mean=point2point_signed(verts_sbj_mean[:,lhand_idx],verts_obj,x_normals=human_normals_mean[:,lhand_idx])[0]
    ...
    def calc_loss(verts,jtr,hand_pose_rec,epoch,left_or_right,human_normals):
        # global WHETHER_MIDTOUCH_LEFT,WHETHER_MIDTOUCH_RIGHT,WHETHER_TOUCH_LEFT,WHETHER_TOUCH_RIGHT
        # EPOCH_SMOOTH=500
        if left_or_right==1:
            WHETHER_TOUCH=WHETHER_TOUCH_RIGHT
            WHETHER_TOUCH_L=WHETHER_TOUCH_RIGHT_L
            frame_times_touch=frame_times_right
            hand_idx=np.load('./assets/smplh_hand_index/rhand_smplh_ids.npy')
        ...
        with torch.enable_grad():       
            ...
            o2h_signed,sbj2obj, o2h_idx, sbj2obj_idx, o2h, sbj2obj_vector = point2point_signed(verts[:,hand_idx], verts_obj,x_normals=human_normals[:,hand_idx],return_vector=True) #y_normals=obj_normals
            min_distance_mean_hand=torch.min(o2h_signed,dim=1)[0]
            min_distance_mean_hand2=torch.min(torch.abs(o2h_signed),dim=1)[0]
            Pene_mask=(min_distance_mean_hand<0.0).float()*(min_distance_mean_hand2<0.02).float()
            ...
            ## Y 0.4 Z 0.5,-0.4
            # original 1.3
            theta_max1_1=torch.tensor([0.6,0.09,0.13]).reshape(1,1,3).float().to(device)
            theta_min1_1=torch.tensor([-0.4,-0.08,-0.2]).reshape(1,1,3).float().to(device)
            loss_pinky_thumb_out_notmid=restrict_angles(thumb_pinky_out_notmid,theta_max1_1,theta_min1_1,mode=1,flag='1')
            
            theta_max1_2=torch.tensor([0.6,0.15,0.12]).reshape(1,1,3).float().to(device)
            theta_min1_2=torch.tensor([-0.2,-0.10,-0.15]).reshape(1,1,3).float().to(device)
            loss_pinky_thumb_out_mid=restrict_angles(thumb_pinky_out_mid,theta_max1_2,theta_min1_2,mode=1,flag='2')

            theta_max1_3=torch.tensor([0.5,0.15,0.10]).reshape(1,1,3).float().to(device)
            ...
            #                                                  -0.1
            theta_max3=torch.tensor([[0.45,0.45,1.5],[0.45,0.45,0.4],[0.45,0.45,1.2]]).reshape(1,3,3).float().to(device)
            
            theta_min3=torch.tensor([[-0.5,-0.5,-0.2],[-0.5,-0.5,-0.4],[-0.5,-0.5,-0.4]]).reshape(1,3,3).float().to(device)

            loss_thumb=restrict_angles(thumb_in,theta_max3,theta_min3,mode=1,flag='4')

            loss_rot_reg =1*(loss_pinky_thumb_out+loss_pinky+loss_thumb)
            ...
            if epoch>100:
                ...
                loss_hand_pose_v_reg=1*smooth_mask(hand_pose_rec,hand_verts,(WHETHER_TOUCH).view(-1,1,1))*0.5
                loss_hand_pose_v_reg+=1*smooth_mask(hand_pose_rec,hand_verts,(WHETHER_OPTIMIZE-WHETHER_TOUCH).view(-1,1,1))
                loss_hand_pose_v_reg+=1*smooth_mask(hand_pose_rec,hand_verts,1-WHETHER_OPTIMIZE.view(-1,1,1))*2

                
                if left_or_right:
                    loss_hand_pose_v_reg+=0.1*torch.sum((hand_pose_rec-hand_pose_init.view(-1,30,3)[:,15:])**2*(1-WHETHER_TOUCH_L).view(-1,1,1))
                else:
                    loss_hand_pose_v_reg+=0.1*torch.sum((hand_pose_rec-hand_pose_init.view(-1,30,3)[:,:15])**2*(1-WHETHER_TOUCH_L).view(-1,1,1))
            ...
            if epoch%2==0:
                delta_temporal_inv_minus=0.1*((delta_inv_minus[1:]-delta_inv_minus[:-1])*WHETHER_TOUCH[1:].view(-1,1,1))**2+((delta_inv_minus[1:]-delta_inv_minus[:-1])*CONTACT_MASK[1:]*WHETHER_TOUCH[1:].view(-1,1,1))**2 ##T,N,3
            else:
                delta_temporal_inv_minus=0.1*((delta_inv_minus[1:]-delta_inv_minus[:-1])*WHETHER_TOUCH[:-1].view(-1,1,1))**2+ ((delta_inv_minus[1:]-delta_inv_minus[:-1])*CONTACT_MASK[1:]*WHETHER_TOUCH[1:].view(-1,1,1))**2##T,N,3
            ...
            ## Not using hand prior loss on Behave, could turn it on if any unnatural hand is observed
            hand_prior_weight = 0
            loss_rot_reg+=hand_prior_weight*torch.mean(torch.abs(hand_prior_loss)**2*WHETHER_TOUCH_L) 
            ...
            loss_v_reg = 1 * (2*loss_hand_pose_v_reg + 0.01*loss_body_v_reg ) +1*loss_rot_reg
            
            
            loss = (
                    loss_dist_o+1*loss_touch+
                    loss_v_reg*1
                    )
            ...
            loss_dict['reg'] = loss_touch.detach().cpu().numpy()#(loss_obj_transl_reg + loss_obj_rot_reg + loss_body_reg + loss_transl_reg + loss_glo_rot_reg + loss_verts_reg).detach().cpu().numpy()
            loss_dict['reg_v'] = loss_v_reg.detach().cpu().numpy()
        return loss,collision_loss,loss_dict
    def calc_loss_common(hand_pose_rec,epoch):
        hand_t=hand_pose_rec.view(-1,90)
        smplx_output = sbj_m(body_pose=body_pose,
                                global_orient=root_tensor,
                                # left_hand_pose=lh_t,
                                # right_hand_pose=rh_t,
                                left_hand_pose=hand_t[:,:45],
                                right_hand_pose=hand_t[:,45:],
                                betas=betas_tensor,
                                transl=trans_tensor)
        verts = (smplx_output.vertices)
        jtr=smplx_output.joints
        all_human_faces=sbj_m.faces
        human_normals=vertex_normals(verts,torch.tensor(all_human_faces.astype(np.float32)).unsqueeze(0).repeat(verts.shape[0],1,1).to(device))

        
        
        loss_left,collision_loss_left,loss_dict_left=calc_loss(verts,jtr,hand_pose_rec[:,:15,:],epoch,0,human_normals)
        loss_right,collision_loss_right,loss_dict_right=calc_loss(verts,jtr,hand_pose_rec[:,15:,:],epoch,1,human_normals)
        loss_dict_all={}
    ...
    hand_pose_rec=Variable(hand_pose_tensor.view(-1,30,3),requires_grad=True)
