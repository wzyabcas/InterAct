...
class CondGaussianDiffusion(nn.Module):
    def __init__(
        ...
        self.use_obj_contact_info = getattr(opt, 'use_obj_contact_info', False)
        self.use_human_contact_info = getattr(opt, 'use_human_contact_info', False)
        self.seperate_obj_contact = getattr(opt, 'seperate_obj_contact', True)
    ...
    def sample(self, x_start, x_cond, cond_mask=None, padding_mask=None):
        ...
        if self.use_obj_contact_info and self.seperate_obj_contact:
            self.contact_encoder.eval()
    ...
    def p_losses(self, x_start, x_cond, t, noise=None, padding_mask=None):
        ...
        if self.loss_weights is None:
            ...
            loss = {}
            pred_motion, gt_motion, start = model_out[..., :553], target[..., :553], 553
            loss_motion = self.loss_fn(pred_motion, gt_motion, reduction = 'none') * padding_mask[:, 0, 1:][:, :, None]
            ...
            if 'contactLabel' in self.loss_weights:
                pred_label, gt_label = model_out[..., start:start+77], target[..., start:start+77]
                start += 77
                loss_label = F.mse_loss(pred_label, gt_label, reduction = 'none') * padding_mask[:, 0, 1:][:, :, None]
                loss_label = reduce(loss_label, 'b ... -> b (...)', 'mean')
                loss_label = loss_label * extract(self.p2_loss_weight, t, loss_label.shape)
                loss['contactLabel'] = loss_label.mean()
            if 'contactVec' in self.loss_weights:
                pred_vec, gt_vec = model_out[..., start:start+77*3], target[..., start:start+77*3]
                start += 77*3
                loss_marker = F.mse_loss(pred_vec, gt_vec, reduction = 'none') * padding_mask[:, 0, 1:][:, :, None]
                loss_marker = reduce(loss_marker, 'b ... -> b (...)', 'mean')
                loss_marker = loss_marker * extract(self.p2_loss_weight, t, loss_marker.shape)
                loss['contactVec'] = loss_marker.mean()
            if 'contactDist' in self.loss_weights:
                pred_dist, gt_dist = model_out[..., start:start+77], target[..., start:start+77]
                start += 77
                loss_dist = F.mse_loss(pred_dist, gt_dist, reduction = 'none') * padding_mask[:, 0, 1:][:, :, None]
                loss_dist = reduce(loss_dist, 'b ... -> b (...)', 'mean')
                loss_dist = loss_dist * extract(self.p2_loss_weight, t, loss_dist.shape)
                loss['contactDist'] = loss_dist.mean()
            return loss
    ...
    def process_cond(self, x_cond):
        if isinstance(x_cond, dict):
            # x_cond is a dictionary - used for my setting, x_cond can 
            obj_bps = x_cond['obj_bps']
            obj_com = x_cond['obj_com']
            if not self.multi_task:
                raise NotImplementedError
            else:
                obj_bps_feat = self.bps_encoder(obj_bps)
                x_cond_processed = torch.cat((obj_com, obj_bps_feat), dim=-1)
        else:
