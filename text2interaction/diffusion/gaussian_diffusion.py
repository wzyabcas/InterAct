...
class GaussianDiffusion:
    ...
    def condition_mean(self, cond_fn, x, t, model_kwargs=None):
        ...
        if is_perturb:
            for _ in range(n_guide_steps):
                loss, grad, loss_list, new_x = cond_fn(x,
                ...
                tao1 = 1
                tao2 = 1
                tao3 = 10  # NOTE Different strengths have a big impact on gudiance and final perfomance, which is a bit tricky and we are still working on this.
    ...
    def training_losses(self, model, x_start, t, model_kwargs=None, noise=None, dataset=None):
        ...
        if self.loss_type == LossType.KL or self.loss_type == LossType.RESCALED_KL:
            ...
            # terms["obj_points"] = model_kwargs['y']['obj_points']
...
class LocalMotionDiffusion(GaussianDiffusion):
    ...
    def training_losses(self, model, mask, t_bar, *args, **kwargs):
        ...
        lambda_contact = 0.9
        lambda_rel = 0.9
        loss_mse = self.masked_l2(items['target'][:, :231], items['pred'][:, :231], mask) + \
            lambda_vel * self.masked_l2(items['target'][:, 231:462], items['pred'][:, 231:462], mask) + \
            lambda_feet * self.masked_l2(items['target'][:, 462:476], items['pred'][:, 462:476], mask) + \
            lambda_obj * self.masked_l2(items['target'][:, 476:485], items['pred'][:, 476:485], mask) + \
            lambda_obj_vel * self.masked_l2(items['target'][:, 485:494], items['pred'][:, 485:494], mask) + \
            lambda_rel * self.masked_l2(items['target'][:, 494:], items['pred'][:, 494:], mask)
        losses = {}
