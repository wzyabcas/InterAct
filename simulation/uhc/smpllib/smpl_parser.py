...
SMPLH_SEGMENT = {
    ...
    "L_Index2":'leftHandIndex1',
    "L_Index3":'leftHandIndex1',
    "L_Middle1":'leftHandIndex1',
    "L_Middle2":'leftHandIndex1',
    "L_Middle3":'leftHandIndex1',
    "L_Pinky1":'leftHandIndex1',
    "L_Pinky2":'leftHandIndex1',
    "L_Pinky3":'leftHandIndex1',
    "L_Ring1":'leftHandIndex1',
    "L_Ring2":'leftHandIndex1',
    "L_Ring3":'leftHandIndex1',
    "L_Thumb1":'leftHand',
    "L_Thumb2":'leftHand',
    "L_Thumb3":'leftHand',
    "R_Index1":'rightHandIndex1',
    "R_Index2":'rightHandIndex1',
    "R_Index3":'rightHandIndex1',
    "R_Middle1":'rightHandIndex1',
    "R_Middle2":'rightHandIndex1',
    "R_Middle3":'rightHandIndex1',
    "R_Pinky1":'rightHandIndex1',
    "R_Pinky2":'rightHandIndex1',
    "R_Pinky3":'rightHandIndex1',
    "R_Ring1":'rightHandIndex1',
    "R_Ring2":'rightHandIndex1',
    "R_Ring3":'rightHandIndex1',
    "R_Thumb1":'rightHand',
    "R_Thumb2":'rightHand',
    "R_Thumb3":'rightHand',
}
...
class SMPL_Parser(_SMPL):
    ...
    def get_offsets(self, zero_pose=None, betas=torch.zeros(1, 10).float(), v_template=None, sbj_vtemp=None, smpl_model=None):
        with torch.no_grad():
...
class SMPLH_Parser(_SMPLH):
    ...
    def __init__(self, *args, **kwargs):
        num_betas_passed = kwargs.get('num_betas', None)
        super(SMPLH_Parser, self).__init__(*args, **kwargs)
        # Check what num_betas the model actually has after initialization
        # shapedirs is typically [num_vertices, 3, num_betas] in smplx
        actual_num_betas = None
        if hasattr(self, 'shapedirs'):
            # shapedirs is [num_vertices, 3, num_betas]
            actual_num_betas = self.shapedirs.shape[2] if len(self.shapedirs.shape) > 2 else None
            
            # If num_betas was specified and is greater than what's in the model, extend shapedirs
            if num_betas_passed is not None and actual_num_betas is not None and num_betas_passed > actual_num_betas:
                print(f"Extending shapedirs from {actual_num_betas} to {num_betas_passed} betas")
                # Create zero padding for additional shape coefficients
                # shapedirs shape is [num_vertices, 3, num_betas]
                num_vertices = self.shapedirs.shape[0]
                num_additional = num_betas_passed - actual_num_betas
                padding = torch.zeros(num_vertices, 3, num_additional, 
                                     dtype=self.shapedirs.dtype, 
                                     device=self.shapedirs.device)
                # Concatenate along the last dimension (betas dimension)
                self.shapedirs = torch.cat([self.shapedirs, padding], dim=2)
                actual_num_betas = num_betas_passed
                print(f"Extended shapedirs.shape to {self.shapedirs.shape}")
        
        print(f"SMPLH_Parser init: num_betas passed={num_betas_passed}, actual shapedirs.shape={self.shapedirs.shape if hasattr(self, 'shapedirs') else 'N/A'}, actual num_betas={actual_num_betas}")
        if num_betas_passed is not None and actual_num_betas is not None and num_betas_passed != actual_num_betas:
            print(f"WARNING: num_betas mismatch! Passed {num_betas_passed} but model has {actual_num_betas}")
        self.device = next(self.parameters()).device
    ...
    def get_joints_verts(self, pose, th_betas=None, th_trans=None):
        ...
        batch_size = pose.shape[0]
        # print(pose.shape, th_betas.shape)
        # if th_trans != None:
        #     print(th_trans.shape)
        print("pose shape: ", pose.shape)
        print("th_betas shape: ", th_betas.shape if th_betas is not None else None)
        smpl_output = self.forward(
            body_pose=pose[:, 3:66],
            global_orient=pose[:, :3],
            L_hand_pose=pose[:, 66:111],
            R_hand_pose=pose[:, 111:156],
            betas=th_betas.repeat(batch_size, 1),
            transl=th_trans,
        )
        vertices = smpl_output.vertices
    ...
    def get_offsets(self, zero_pose=None, betas=torch.zeros(1, 16).float(), v_template=None, sbj_vtemp=None, smpl_model=None):
        with torch.no_grad():
            if zero_pose is None:
                verts, Jtr = self.get_joints_verts(self.zero_pose, th_betas=betas)
            else:
            ...
            joint_pos = Jtr[0].numpy()
            smpl_joint_parents = self.parents.cpu().numpy()
            joint_offsets = {joint_names[i]: offsets_smpl[i] for i in range(len(joint_names))}
            # print(jts_np, joint_offsets)
            parents_dict = {joint_names[i]: joint_names[parents[i]] for i in range(len(joint_names))}
...
class SMPLX_Parser(_SMPLX):
    ...
    def __init__(self, *args, **kwargs):
        ...
        print(self.joint_to_use)
    def forward(self, *args, **kwargs):
        smpl_output = super(SMPLX_Parser, self).forward(*args, **kwargs)
        return smpl_output

    def get_joints_verts(self, pose=None, th_betas=None, th_trans=None, smpl_model=None):
        """
        Pose should be batch_size x 156
        """
        ...
        if th_betas is not None:
            th_betas = th_betas.float()
        # print(th_betas.shape, th_trans.shape, pose.shape)
        batch_size = pose.shape[0]
        # print(batch_size)
        ...
        joints = smpl_output.joints
        #         return vertices, joints
        print(joints[0, 28] - joints[0, 20])
        return vertices, joints
    
     

    def get_offsets(self, v_template=None, zero_pose=None, betas=torch.zeros(1, 26).float(), sbj_vtemp=None, smpl_model=None):
        if not v_template is None:
            self.v_template = v_template
        with torch.no_grad():
            if zero_pose is None:
                verts, Jtr = self.get_joints_verts(self.zero_pose, th_betas=betas, smpl_model=smpl_model)
            else:
                verts, Jtr = self.get_joints_verts(zero_pose, th_betas=betas, smpl_model=smpl_model)
            verts_np = verts.detach().cpu().numpy()
            jts_np = Jtr.detach().cpu().numpy()
            parents = self.parents.cpu().numpy()
            offsets_smpl = [np.array([0, 0, 0])]
            ...
            offsets_smpl = np.array(offsets_smpl)
            joint_names = self.joint_names
            # print(parents, offsets_smpl.shape, joint_names)
            joint_pos = Jtr[0].numpy()
            smpl_joint_parents = self.parents.cpu().numpy()
            joint_offsets = {joint_names[i]: offsets_smpl[SMPLX_BONE_ORDER_NAMES.index(joint_names[i])] for i in range(len(joint_names))}
            # print(jts_np, joint_offsets)
            parents_dict = {joint_names[i]: joint_names[SMPLH_BONE_ORDER_NAMES.index(SMPLX_BONE_ORDER_NAMES[parents[SMPLX_BONE_ORDER_NAMES.index(joint_names[i])]])] for i in range(1, len(joint_names))}
            # print(joint_names, parents_dict)
            parents_dict["Hips"] = "None"
            channels = ["z", "y", "x"]
            skin_weights = self.lbs_weights.numpy()
            joints = np.concatenate([jts_np[0][:22], jts_np[0][25:55]], axis=0)
            skin_weights = np.concatenate([skin_weights[:, :22], skin_weights[:, 25:55]], axis=1)
            return (verts[0], joints, skin_weights, self.joint_names, joint_offsets, parents_dict, channels, self.joint_range)

    def get_mesh_offsets(self, sbj_vtemp=None, smpl_model=None):
        # if not v_template is None:
        #     self.v_template = v_template
        with torch.no_grad():
            #             joint_names = self.joint_names
            joint_names = SMPLX_BONE_ORDER_NAMES
            verts, Jtr = self.get_joints_verts(smpl_model=smpl_model)
            verts_np = verts.detach().cpu().numpy()
            jts_np = Jtr.detach().cpu().numpy()
            parents = self.parents.cpu().numpy()
            offsets_smpl = [np.array([0, 0, 0])]
