...
def find_closest_points_and_vectors(A, B):
    T, N, _ = A.shape
    closest_indices = np.zeros((T, N), dtype=int)
    closest_vectors = np.zeros((T, N, 3))  # 存储最近邻点之间的向量

    for t in range(T):
        tree = cKDTree(B[t])
        # 查询最近的点索引和对应的距离
        _, indices = tree.query(A[t], k=1)
        closest_indices[t] = indices
        # 计算A中每个点到B中最近点的向量
        closest_vectors[t] = B[t][indices] - A[t]
