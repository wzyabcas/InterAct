...
def process_sequence(dataset, name, MOTION_PATH, dataset_path, NEW_MOTION_PATH, OBJECT_PATH, progress_lock, progress_bar):
    ...
    try:
        ...
        # object in the canonical system (support both single and multi-object files)
        for entry in object_entries:
            ...
            n_frames = min(poses.shape[0], obj_angles.shape[0], obj_trans.shape[0])
            new_obj_angles = []
            new_obj_trans = []
        ...
        human_min_y = verts[:30, ..., 1].min()
        object_min_y = None
        for obj_entry in new_objects:
            mesh_obj_path = resolve_object_mesh_path(OBJECT_PATH, obj_entry["name"], obj_entry["filename"])
            mesh_obj = trimesh.load(mesh_obj_path, force='mesh')
            obj_angles_eval = obj_entry["data"]["angles"]
            obj_trans_eval = obj_entry["data"]["trans"]
            n_eval = min(30, obj_angles_eval.shape[0], obj_trans_eval.shape[0])
            if n_eval <= 0:
                continue
            angle_matrix = Rotation.from_rotvec(obj_angles_eval[:n_eval]).as_matrix()
            obj_verts = mesh_obj.vertices[None, ...]
            obj_verts = np.matmul(obj_verts, np.transpose(angle_matrix, (0, 2, 1))) + obj_trans_eval[:n_eval, None, :]
            cur_min_y = obj_verts[..., 1].min()
            object_min_y = cur_min_y if object_min_y is None else min(object_min_y, cur_min_y)

        diff_fix = human_min_y if object_min_y is None else min(human_min_y, object_min_y)
        new_trans = np.array(new_trans)
        new_trans[..., 1] -= diff_fix
        for obj_entry in new_objects:
            obj_entry["data"]["trans"][..., 1] -= diff_fix
        ...
        for obj_entry in new_objects:
            np.savez(
                os.path.join(NEW_MOTION_PATH, name, obj_entry["filename"]),
                **obj_entry["data"]
            )
